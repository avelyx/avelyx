<!-- ANIMATED HEADER — dark/light mode responsive -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="./header-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="./header-light.svg">
  <img src="./header-dark.svg" alt="AVELYX" width="100%">
</picture>

<div align="center">

<br>

<!-- TYPING ANIMATION -->
<a href="https://avelyx.ch">
<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=18&duration=3500&pause=1200&color=8B5CF6&center=true&vCenter=true&multiline=false&repeat=true&width=520&height=30&lines=%24+building+systems+that+think+_" alt="Typing SVG" />
</a>

<br><br>

<!-- SOCIAL LINKS -->
<a href="https://avelyx.ch">
<img src="https://img.shields.io/badge/avelyx.ch-000?style=for-the-badge&logo=vercel&logoColor=white" />
</a>
&#8287;
<a href="https://cloudelevation.ch">
<img src="https://img.shields.io/badge/swiss_cloud_elevation-000?style=for-the-badge&logo=icloud&logoColor=white" />
</a>
&#8287;
<a href="mailto:info@avelyx.ch">
<img src="https://img.shields.io/badge/contact-000?style=for-the-badge&logo=maildotru&logoColor=white" />
</a>

</div>

<br>

<!-- DIVIDER -->
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%">

<br>

<!-- ABOUT SECTION -->
<table align="center" border="0" cellpadding="0" cellspacing="0">
<tr>
<td width="55%" valign="top">

<br>

```js
const avelyx = {
  location: "Zurich, Switzerland",
  focus: [
    "AI-Powered Web Development",
    "Cloud Architecture & Automation",
    "Algorithmic Trading Systems"
  ],
  currently: "Building AI pipelines that
               generate production websites",
  philosophy: "Ship fast. Design slow."
};
```

</td>
<td width="45%" valign="top">

<br>

<div align="center">

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=avelyx&layout=donut-vertical&hide_border=true&bg_color=00000000&title_color=8b5cf6&text_color=888888&hide=jupyter%20notebook&langs_count=6" width="280" />

</div>

</td>
</tr>
</table>

<br>

<!-- DIVIDER -->
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%">

<br>

<!-- TECH STACK -->
<div align="center">

<br>

**`>_ tech stack`**

<br>

<table border="0" cellpadding="0" cellspacing="0">
<tr>
<td align="center" width="140">
<br>
<img src="https://skillicons.dev/icons?i=html" width="40" /><br>
<sub><b>HTML</b></sub><br><br>
</td>
<td align="center" width="140">
<br>
<img src="https://skillicons.dev/icons?i=css" width="40" /><br>
<sub><b>CSS</b></sub><br><br>
</td>
<td align="center" width="140">
<br>
<img src="https://skillicons.dev/icons?i=js" width="40" /><br>
<sub><b>JavaScript</b></sub><br><br>
</td>
<td align="center" width="140">
<br>
<img src="https://skillicons.dev/icons?i=ts" width="40" /><br>
<sub><b>TypeScript</b></sub><br><br>
</td>
<td align="center" width="140">
<br>
<img src="https://skillicons.dev/icons?i=react" width="40" /><br>
<sub><b>React</b></sub><br><br>
</td>
<td align="center" width="140">
<br>
<img src="https://skillicons.dev/icons?i=nextjs" width="40" /><br>
<sub><b>Next.js</b></sub><br><br>
</td>
</tr>
<tr>
<td align="center" width="140">
<br>
<img src="https://skillicons.dev/icons?i=python" width="40" /><br>
<sub><b>Python</b></sub><br><br>
</td>
<td align="center" width="140">
<br>
<img src="https://skillicons.dev/icons?i=nodejs" width="40" /><br>
<sub><b>Node.js</b></sub><br><br>
</td>
<td align="center" width="140">
<br>
<img src="https://skillicons.dev/icons?i=supabase" width="40" /><br>
<sub><b>Supabase</b></sub><br><br>
</td>
<td align="center" width="140">
<br>
<img src="https://skillicons.dev/icons?i=docker" width="40" /><br>
<sub><b>Docker</b></sub><br><br>
</td>
<td align="center" width="140">
<br>
<img src="https://skillicons.dev/icons?i=azure" width="40" /><br>
<sub><b>Azure</b></sub><br><br>
</td>
<td align="center" width="140">
<br>
<img src="https://skillicons.dev/icons?i=figma" width="40" /><br>
<sub><b>Figma</b></sub><br><br>
</td>
</tr>
</table>

<br>

<img src="https://img.shields.io/badge/Claude_API-191919?style=flat-square&logo=anthropic&logoColor=white" />
<img src="https://img.shields.io/badge/Gemini-4285F4?style=flat-square&logo=google&logoColor=white" />
<img src="https://img.shields.io/badge/GSAP-88CE02?style=flat-square&logo=greensock&logoColor=black" />
<img src="https://img.shields.io/badge/Three.js-000000?style=flat-square&logo=threedotjs&logoColor=white" />
<img src="https://img.shields.io/badge/n8n-EA4B71?style=flat-square&logo=n8n&logoColor=white" />
<img src="https://img.shields.io/badge/Hetzner-D50C2D?style=flat-square&logo=hetzner&logoColor=white" />
<img src="https://img.shields.io/badge/Remotion-5B37B7?style=flat-square&logo=react&logoColor=white" />
<img src="https://img.shields.io/badge/Swift-F05138?style=flat-square&logo=swift&logoColor=white" />

</div>

<br>

<!-- DIVIDER -->
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%">

<br>

<!-- PROJECTS -->
<div align="center">

<br>

**`>_ what I build`**

<br>

</div>

<table align="center" border="0" cellpadding="0" cellspacing="0">
<tr>
<td width="50%" valign="top">

<div align="center">

**Avelyx Studio**
<br>
<sub>AI-powered website configurator for Swiss SMBs.<br>Configure. Generate. Launch.</sub>

<br>

`Claude` `Supabase` `Stripe` `GSAP`

</div>

</td>
<td width="50%" valign="top">

<div align="center">

**4-Pass Demo Generator**
<br>
<sub>AI pipeline that builds unique production websites<br>from scratch — blueprint to pixel-perfect output.</sub>

<br>

`Opus` `Sonnet` `n8n` `Puppeteer`

</div>

</td>
</tr>
<tr>
<td width="50%" valign="top">

<br>

<div align="center">

**Algorithmic Trading**
<br>
<sub>Python bot + Swift iOS dashboard.<br>Real-time analytics and automated execution.</sub>

<br>

`Python` `Swift` `pybit` `pandas`

</div>

</td>
<td width="50%" valign="top">

<br>

<div align="center">

**Swiss Cloud Elevation**
<br>
<sub>M365 tenant automation, AVD deployments,<br>and infrastructure as code.</sub>

<br>

`PowerShell` `Bicep` `Azure` `M365`

</div>

</td>
</tr>
</table>

<br>

<!-- DIVIDER -->
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%">

<br>

<!-- GITHUB STATS -->
<div align="center">

<br>

**`>_ metrics`**

<br>

<p>
<img width="49%" src="https://github-readme-stats.vercel.app/api?username=avelyx&show_icons=true&hide_border=true&bg_color=00000000&title_color=8b5cf6&text_color=888888&icon_color=8b5cf6&ring_color=8b5cf6&count_private=true&include_all_commits=true" />
<img width="49%" src="https://github-readme-streak-stats.herokuapp.com?user=avelyx&hide_border=true&background=00000000&ring=8B5CF6&fire=8B5CF6&currStreakLabel=8B5CF6&sideLabels=888888&currStreakNum=FFFFFF&sideNums=888888&dates=555555&stroke=00000000" />
</p>

<br>

<!-- ACTIVITY GRAPH -->
<img src="https://github-readme-activity-graph.vercel.app/graph?username=avelyx&bg_color=00000000&color=888888&line=8b5cf6&point=ffffff&area=true&area_color=8b5cf680&hide_border=true&custom_title=contribution%20timeline" width="96%" />

</div>

<br>

<!-- DIVIDER -->
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%">

<br>

<!-- SNAKE -->
<div align="center">

<br>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/avelyx/avelyx/snake/github-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/avelyx/avelyx/snake/github-snake.svg" />
  <img alt="github-snake" src="https://raw.githubusercontent.com/avelyx/avelyx/snake/github-snake-dark.svg" width="100%" />
</picture>

</div>

<br>

<!-- FOOTER -->
<div align="center">

<img src="https://komarev.com/ghpvc/?username=avelyx&color=8b5cf6&style=flat-square&label=visitors" />

<br><br>

```
design is not decoration — it's communication
code is not logic — it's craft
ai is not replacement — it's amplification
```

<br>

<sub>zurich · switzerland</sub>

<br>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0a0a0a,50:1a1a2e,100:8b5cf620&height=100&section=footer&fontSize=0" width="100%"/>

</div>
